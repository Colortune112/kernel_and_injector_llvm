...
)
{
    ...
    IoSetCompletionRoutine(
        AsyncContext->Irp,
        reinterpret_cast< PIO_COMPLETION_ROUTINE >( &KspAsyncContextCompletionRoutine ),
        &AsyncContext->CompletionEvent,
        TRUE,
        TRUE,
        TRUE
...
)
{
    ...
    PKSOCKET NewSocket = reinterpret_cast< PKSOCKET >( ExAllocatePoolWithTag( NonPagedPool, sizeof( KSOCKET ), MEMORY_TAG ) );
...
)
{
    ...
    if ( NT_SUCCESS( Status ) )
    {
        PKSOCKET KNewSocket = reinterpret_cast< PKSOCKET >( ExAllocatePoolWithTag( NonPagedPool, sizeof( KSOCKET ), MEMORY_TAG ) );
